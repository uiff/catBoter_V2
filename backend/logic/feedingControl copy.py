...
def process_single_feeding(fütterung, current_time):
    ...
    try:
        ...
        # Prüfe Zeitbedingungen
        if fütterungszeit > current_time:
            logging.debug(f"Überspringe zukünftige Fütterungszeit: {fütterung['time']}")
            return False
        ...
        logging.info(f"Starte Fütterung um {fütterung['time']} mit Zielgewicht: {target_weight}g")
        ...
        if success:
            logging.info(f"Fütterung erfolgreich: {message}")
        else:
            logging.warning(f"Fütterung fehlgeschlagen: {message}")
